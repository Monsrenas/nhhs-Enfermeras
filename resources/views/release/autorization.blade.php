...
<div class="container" style="text-align: justify; width: 80%">
	...
	<p>
		...
		<input type="radio" name="pag[level]" id="level1" style="margin-right: .8em; margin-left: .8em;" value="1" {{($d['level']??"")=="1"? "checked":""}} >
		{{trans('application.authorize2')}}
		<input type="radio" name="pag[level]" id="level2" style="margin-right: .8em; margin-left: .8em;" value="2" {{($d['level']??"")=="2"? "checked":""}}>
	...
	<div class="form-group row">
		@foreach($items as $item)
			<input class="col-lg-3 col-sm-6 form-control form-control-sms" type="text" name="pag[{{$item}}]" value="{{$d[$item]??""}}" 
	...
	<div class="form-group row">
		@foreach($prevs as $item)
			...
			<br>
			<textarea  name="pag[{{$item}}]">{{$d[$item]??""}}</textarea>
			<br><br>
			</p>
		@endforeach	
	</div>	
	...
	<div class="form-group row">
		...
		<div class="col-lg-4 col-sm-6">		
			<input class="form-control form-control-sms"  id="mpy{{$item}}" type="<?php echo ($item=="DOB")?"date":"text"?>" name="pag[sig{{$item}}]" value="{{$d["sig".$item]??""}}">
			 <label>{{trans('application.'.$item)}}</label>
		</div>	 
		@endforeach
		...
		<div class="col-lg-4 col-sm-6">		
			 <label>{{trans('application.sex')}}:</label>
			  <select  name="pag[sex]" >
                    ...
                    <option  value=1 {{($d["sex"]??"")=="1"? "selected":""}} > {{trans('application.female')}}</option>
                    <option  value=2 {{($d["sex"]??"")=="2"? "selected":""}}> {{trans('application.male')}}</option>
              </select>
		</div>
	...
	<div class="form-group row">
		...
		<div class="col-lg-3 col-sm-6">		
			<input class="form-control form-control-sms" type="text" name="pag[{{$item}}]" value="{{$d[$item]??""}}">
			 <label>{{trans('application.'.$item)}}</label>
		</div>	 
		@endforeach
	</div> 		
	...
	<br>
	<div class="form-group row">
		<input type="checkbox" name="pag[signature]" required {{($d['signature']??"")=="on"? "checked":""}}>
		...
		<div class="col-lg-5 col-sm-6">		
		    ...
		    @endif 
			<input class="form-control form-control-sms" type="<?php echo ($item=="Date")?"date":"text"?>" name="pag[sig{{$item}}]" value="{{$d["sig".$item]??""}}" required>
			 <label>{{trans('application.'.$item)}}</label>
		</div>	 
		@endforeach
	</div> 	
